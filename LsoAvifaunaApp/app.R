...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("vars_g", "Group by e.g. (species,decimalLongitude,decimalLatitude,locality,stateProvince etc simultaneously) or you can select one variable", names(nonyana), multiple =
                    TRUE),
...
server <- function(input, output, session) {
  ...
  output$dataviz = renderPlot({
    ...
    if(input$BirdViz == "Family Frequency") {
      z<-nonyana %>%
        ...
        theme(axis.text.x = element_text(angle= 90, hjust=0)) + xlab("Lesotho Bird Families") + ylab("Frequency")+facet_wrap(~stateProvince, ncol=2)}


    if(input$BirdViz == "Order Frequency") {
      z<-nonyana %>%
        group_by(stateProvince,order) %>%
        summarise(count=n()) %>%
        mutate(freq=count/sum(count)) %>%
        filter(count>2) %>%
        ggplot(aes(x=reorder(order, -freq),y=freq)) + geom_bar(stat="identity", fill= "tomato") +theme_bw(base_size=10) +
        theme(axis.text.x = element_text(angle= 90, hjust=0)) + xlab("Lesotho Bird Orders") + ylab("Frequency")+facet_wrap(~stateProvince, ncol=2)}
    ...
    if(input$BirdViz == "Bird Genus Frequency") {
      z<-nonyana %>%
        ...
        theme(axis.text.x = element_text(angle= 90, hjust=0)) + xlab("Lesotho Bird Genuses") + ylab("Frequency")+facet_wrap(~stateProvince, ncol=2)}
    ...
    if(input$BirdViz == "Bird Species Frequency") {
      z<-nonyana %>%
        ...
        theme(axis.text.x = element_text(angle= 90, hjust=0)) + xlab("Lesotho Bird Species") + ylab("Frequency")+facet_wrap(~stateProvince, ncol=2)}
    z
  ...
  output$BirdMapViz <- renderLeaflet({
    ...
    nonyana %>%
        ...
        addCircleMarkers(lng = ~decimalLongitude, lat = ~decimalLatitude,stroke = TRUE, color = "seagreen", popup = ~species, labelOptions = labelOptions(noHide = TRUE))}
  })
